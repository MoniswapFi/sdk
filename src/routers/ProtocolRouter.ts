...
export class ProtocolRouter {
  ...
  async quoteRemoveLiquidity(
    ...
    try {
      ...
      const amounts = await caller.ethCall<[bigint, bigint]>(protocolRouterAbi, this.address, "quoteRemoveLiquidity", [
        tokenA.address,
        tokenB.address,
        stable,
        factory,
  ...
  async addLiquidity(
    ...
    try {
      ...
      const txHash = await caller.submitTransaction(
        ...
        this.address,
        "addLiquidity",
        [tokenA.address, tokenB.address, stable, amountADesired, amountBDesired, BigInt(0), BigInt(0), to, deadline],
        value,
        privateKey,
        gas,
        gasPrice,
  ...
  async swap(
    ...
    try {
      ...
      if (routes[0].from === ETH_ADDRESS) {
        ...
        method = feeOnTransferTokens ? "swapExactETHForTokensSupportingFeeOnTransferTokens" : "swapExactETHForTokens";
        ...
        method = feeOnTransferTokens ? "swapExactTokensForETHSupportingFeeOnTransferTokens" : "swapExactTokensForETH";
